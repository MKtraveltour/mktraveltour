import json

data = {
  "articles": [
    {
      "id": "004",
      "date": "2026-06-22",
      "title": "中の人2号です",
      "text": "はじめまして！\n平素よりMKタクシーをご利用いただきありがとうございます。\n大好評(？)だったホテル様向けのHPでも一緒にやってきた上司と再び、このHPを担当させて頂きます。\nいつもお世話になっているお客様だけに少しだけ特別な情報をお届けしますので、どうぞよろしくお願いいたします！",
      "photos": [],
      "author": "RYUTARO",
      "category": "New！"
    },
    {
      "id": "003",
      "date": "2026-06-22",
      "title": "仲間が増えました",
      "text": "ホームページ更新作業にあたり一人では心もとないということで更新担当を1人新たに勧誘しました。\n名前はRYUTARO。部署のエンターテイナーです。\nより楽しい記事を展開してくれると思います！",
      "photos": [],
      "author": "H.I",
      "category": "New！"
    },
    {
      "id": "002",
      "date": "2026-05-24",
      "title": "美山のとある場所より",
      "text": "まだ知られていない秘境を求めて。\n上司から送信されてきたのは見たことのないかやぶき屋根の住宅。\nなになに日本最古の農家型住宅…？重要文化財？！\n見覚えのあるかやぶき屋根よりもさらに低いような…\nこれは気になる…果たしてツアーに出来るでしょうか。",
      "photos": [
        "https://raw.githubusercontent.com/MKtraveltour/mktraveltour/main/20260524%20ishidake%20kimura.jpg"
      ],
      "author": "H.I",
      "category": "企画のたまご"
    },
    {
      "id": "001",
      "date": "2026-06-16",
      "title": "担当よりご挨拶",
      "text": "はじめまして！\nMKトラベルをご利用いただいたみなさまにもっとDeepな京都を！\nそんな思いで新たにホームページがスタートしました。\n初めてのことだらけ、ぴかぴかのたまごからスタートします！\nこんなことがしたいな、という企画のたまごからひよこの成長記録のように新しい企画ツアーの進捗状況や当日のツアーの魅力をドライバーからおすそ分けしてもらったり。\nいろいろなツアーの様子をみなさまにお届けしたいと思っています。\nこれからどうぞよろしくお願いします！",
      "photos": [],
      "author": "H.I",
      "category": "New！"
    }
  ]
}

with open("articles.json", "w", encoding="utf-8") as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

print("✅ articles.json を UTF-8 で作成しました")
