...
class TourEditor:
    ...
    def _build_report_tab(self, parent):
        ...
        # ヘッダー写真URL
        lbl2("ヘッダー写真URL（レポートページの背景）")
        self.report_hero_var = tk.StringVar()
        tk.Entry(right, textvariable=self.report_hero_var, font=("", 10),
                 bg="#fff", relief="solid", bd=1).pack(fill="x", padx=4)

        # 煽り文句
        lbl2("ヘッダー煽り文句（例: はじめてだからこそ、特別に。）")
        self.report_catch_var = tk.StringVar()
        tk.Entry(right, textvariable=self.report_catch_var, font=("", 11),
                 bg="#fff", relief="solid", bd=1).pack(fill="x", padx=4)

        # 撮影日
        lbl2("撮影日（例: 2025年10月22日（水））")
        self.report_shot_date_var = tk.StringVar()
        tk.Entry(right, textvariable=self.report_shot_date_var, font=("", 10),
                 bg="#fff", relief="solid", bd=1).pack(fill="x", padx=4)
    ...
    def _on_report_select(self, event):
        ...
        self.report_desc_box.delete("1.0", "end")
        self.report_desc_box.insert("1.0", r.get("desc", ""))
    ...
    def _save_report(self):
        ...
        photos = [self._fix_photo_url(p.strip())
                  ...
                  if p.strip()]
        desc      = self.report_desc_box.get("1.0", "end").strip()
        hero      = self._fix_photo_url(self.report_hero_var.get().strip())
        catch     = self.report_catch_var.get().strip()
        shot_date = self.report_shot_date_var.get().strip()
